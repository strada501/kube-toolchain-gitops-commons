...
# Get some variables from deployment
...
PIPELINE_IMAGE_URL=$(yq read --doc $DEPLOYMENT_DOC_INDEX $DEPLOYMENT_FILE "spec.template.spec.containers[0].image")
if [ -z "$PIPELINE_IMAGE_URL" ]; then
  echo "No Container Imange definition found in $DEPLOYMENT_FILE."
  exit 1
fi
